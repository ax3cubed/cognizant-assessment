...
public class TaskInitialize implements CommandLineRunner {
    ...
    private List<Task> getTasks(){
      return   List.of(
                new Task( "Longest Common Prefix","Write a function to find the longest common prefix string amongst an array of strings.\n" +
                        "\n" +
                        "If there is no common prefix, return an empty string","[\"flower\",\"flow\",\"flight\"]","fl"),
                new Task( "Reverse Integer","Given a 32-bit signed integer, reverse digits of an integer.","123","321"),
                new Task( "Integer to Roman","Roman numerals are represented by seven different symbols: I, V, X, L, C, D and M.","58","LVIII"),
                new Task("Merge k Sorted Lists","You are given an array of k linked-lists lists, each linked-list is sorted in ascending order.","[[1,4,5],[1,3,4],[2,6]]","[1,1,2,3,4,4,5,6]")
        );
    }
